...
if !exists("b:tt2_syn_inc_js")
    let b:tt2_syn_inc_js = 1
endif
...
syn cluster tt2_top_cluster contains=tt2_perlcode,tt2_javascript,tt2_tag_region
...
if exists("b:tt2_syn_tags")
    ...
    while stridx(s:str,' ') > 0
        ...
        "Include JavaScript syntax when BLOCK inline_js
        if b:tt2_syn_inc_js
            syn include @javaScript $VIMRUNTIME/syntax/javascript.vim
            unlet b:current_syntax
            exec 'syn region tt2_javascript '.
                        \ 'start=+\(BLOCK inline_js\s*[-]\=' . s:ed . '\(\n\)\=\)\@<=+ ' .
                        \ 'end=+' . s:st . '[-]\=\s*END+me=s-1 contains=@javaScript keepend'
        endif
    ...
    if b:tt2_syn_inc_js
        syn include @javaScript $VIMRUNTIME/syntax/javascript.vim
        unlet b:current_syntax
        syn region tt2_javascript
                    \ start=+\(BLOCK inline_js\s*[-]\=%]\(\n\)\=\)\@<=+
                    \ end=+\[%[-]\=\s*END+me=s-1
                    \ contains=@javaScript keepend
    endif
endif
...
hi def link tt2_tag_region          Number      " [%( ~ )
hi def link tt2_directive           Number      " [% ( PROCESS )
hi def link tt2_blockname           String      " [% PROCESS ( "" ) %]
"hi def link tt2_variable           Comment     " [% PROCESS "" ( var1 ) => var2 %]/[% ( variable ) %]
hi def link tt2_ivariable           Comment     " [% "string ( ${variable} )" %]
hi def link tt2_operator            Function    " [% PROCESS "" var1 ( => ) var2( , ) %]
hi def link tt2_string_qq           String      " [% PROCESS "" var1 => ( "abc" ) %]
hi def link tt2_string_q            String      " [% PROCESS "" var1 => ( 'abc' ) %]
hi def link tt2_func                Function    " [% ( func )('stuff') %]
hi def link tt2_number              Identifier  " [% var = ( 1 ) %]
hi def link tt2_commentblock_region Comment     " [%( # COMMENT )
